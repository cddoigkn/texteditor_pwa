...
export const putDb = async (content) => {
  ...
  const currentDB = await openDB(DB_NAME, 1);

  // TODO: Now create a variable for the transaction; again, this will be referenced below.
  const currentTxt = currentDB.transaction(DB_NAME, 'readwrite');

  // TODO: Now create a variable for the store
  const currentStore = currentTxt.objectStore(DB_NAME);

  const request = currentStore.put({ id: 1, value: content });
...
export const getDb = async () => {
  ...
  const currentDB = await openDB(DB_NAME, 1);
  const currentTxt = currentDB.transaction(DB_NAME, 'readonly');
  const currentStore = currentTxt.objectStore(DB_NAME);
  // Leave the rest as-is
  const request = currentStore.get(1);
  const result = await request;
  result
